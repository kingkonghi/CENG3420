 ...
 * 	 * hard-wired to zero
 * 	 	 */
	printf("Lab3-1 assignment: hard-wired to zero\n");
 ...
 *           */
    ...
    if(ird ==1){ NEXT_LATCHES.STATE_NUMBER = mask_val(CURRENT_LATCHES.IR,6,0);
    }
    else{ if((CURRENT_LATCHES.MICROINSTRUCTION[J2] ==0) && (CURRENT_LATCHES.READY ==1)) j=j+(CURRENT_LATCHES.READY<<2);
		 if((CURRENT_LATCHES.MICROINSTRUCTION[J3] ==0) && (_B ==1)) j = j+ (_B<<3);
		NEXT_LATCHES.STATE_NUMBER = j ;

	   	}
	printf("NEXT STATE: %d ,CURRENT STATE: %d\n READY: %d\n",NEXT_LATCHES.STATE_NUMBER,CURRENT_LATCHES.STATE_NUMBER,
								CURRENT_LATCHES.READY);
	memcpy(NEXT_LATCHES.MICROINSTRUCTION,
        CONTROL_STORE[NEXT_LATCHES.STATE_NUMBER], sizeof(int) * CONTROL_STORE_BITS);
...
void cycle_memory() {
    ...
    if (mio_en) {
        ...
        if (W) {
            ...
            write_memory(blockDATASIZEMUX(get_DATASIZE(CURRENT_LATCHES.MICROINSTRUCTION),mask_val(CURRENT_LATCHES.IR,14,12),0),
			CURRENT_LATCHES.MAR,
			CURRENT_LATCHES.MDR) ;
        } else {
            /* Read */
	...
	if(get_LD_PC(CURRENT_LATCHES.MICROINSTRUCTION)==0)MemOut = read_memory(blockDATASIZEMUX(get_DATASIZE(CURRENT_LATCHES.MICROINSTRUCTION),
                                                        mask_val(CURRENT_LATCHES.IR,14,12),0),CURRENT_LATCHES.MAR);
...
void eval_bus_drivers() {
     ...
     *  Lab3-2 assignment
     */
	int MARMUXa,MDRMUXa,ADDR1MUXa,ADDR2MUXa,ALUa,RS2MUXa,PCMUXa,IRa;
	int A,B,ADD,RS1,RS2,i,sum =0,R1out,R2out;
	...
	IRa = CURRENT_LATCHES.IR;
	MARMUXa = get_MARMUX(CURRENT_LATCHES.MICROINSTRUCTION);
	MDRMUXa = get_MDRMUX(CURRENT_LATCHES.MICROINSTRUCTION);
	ADDR1MUXa = get_ADDR1MUX(CURRENT_LATCHES.MICROINSTRUCTION);
	ADDR2MUXa = get_ADDR2MUX(CURRENT_LATCHES.MICROINSTRUCTION);

	PCMUXa =  get_PCMUX(CURRENT_LATCHES.MICROINSTRUCTION);
	RS2MUXa = get_RS2MUX(CURRENT_LATCHES.MICROINSTRUCTION);
	ADD= (blockADDR2MUX(ADDR2MUXa,
			0,
			blockSEXT(mask_val(IRa,31,20),12),
                        blockSEXT(blockStypeImm(mask_val(IRa,11,7),
                                      mask_val(IRa,31,25)),12),
			blockSEXT(blockJtypeImm(mask_val(IRa,31,31),
                            mask_val(IRa,30,21),
                            mask_val(IRa,20,20),
                            mask_val(IRa,19,12)),20
                        )
		)
        	+
	     (blockADDR1MUX(ADDR1MUXa,
			0,
			CURRENT_LATCHES.PC,
                        blockRS1En(get_RS1En(CURRENT_LATCHES.MICROINSTRUCTION),
					0,
					CURRENT_LATCHES.REGS[mask_val(IRa,19,15)]),
			blockSEXT(blockBtypeImm( mask_val(IRa,7,7),mask_val(IRa,11,8),mask_val(IRa,30,25),mask_val(IRa,31,31)),12)
			)
		)
	);
	int RS2a =  blockRS2MUX(RS2MUXa,
                                blockRS2En(
                                	get_RS2En(CURRENT_LATCHES.MICROINSTRUCTION),
                                        0,
	                                CURRENT_LATCHES.REGS[mask_val(IRa, 24, 20)]),
                                blockSEXT(mask_val(IRa, 31, 20),12)
                              );
	valueOfGateMAR= blockMARMUX(MARMUXa,ADD,mask_val(IRa,31,12)<<20);
	valueOfGatePC= CURRENT_LATCHES.PC;
	valueOfGateMDR = CURRENT_LATCHES.MDR;
	valueOfGateRS2= blockRS2En(get_RS2En(CURRENT_LATCHES.MICROINSTRUCTION),
					0,
                                        CURRENT_LATCHES.REGS[mask_val(IRa, 24, 20)]
                                );
	valueOfGateALUSHF=blockALUSHFMUX(mask_val(IRa,14,12),
				blockALU(mask_val(IRa, 14, 12),
					mask_val(IRa, 31, 25),
                                	blockRS1En(
...
void drive_bus() {
	...
	if (get_GateMAR(CURRENT_LATCHES.MICROINSTRUCTION) ==1){BUS = valueOfGateMAR;}
	else if(get_GatePC(CURRENT_LATCHES.MICROINSTRUCTION) ==1){BUS = valueOfGatePC;}
	else if(get_GateMDR(CURRENT_LATCHES.MICROINSTRUCTION) ==1){BUS = valueOfGateMDR;}
	else if(get_GateRS2(CURRENT_LATCHES.MICROINSTRUCTION) ==1){BUS = valueOfGateRS2;}
	else if(get_GateALUSHF(CURRENT_LATCHES.MICROINSTRUCTION) ==1){BUS = valueOfGateALUSHF;}
	else BUS = 0;
...
void latch_datapath_values() {
    ...
    if (get_LD_PC(CURRENT_LATCHES.MICROINSTRUCTION)) {
        ...
        */
        ...
        printf("Lab3-2 LD_PC");
	NEXT_LATCHES.PC = blockPCMUX(get_PCMUX(CURRENT_LATCHES.MICROINSTRUCTION),CURRENT_LATCHES.PC+4,BUS);
