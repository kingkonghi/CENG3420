...
int cmd2code(int lineNum, int cmdNum, char *pOpcode, char *pArg1, char *pArg2,
	...
	if (!strncmp(pOpcode, "addi", 4) && strlen(pOpcode) == 4) {
		...
		code += (1<<12);
		code += (reg2num(pArg2, lineNum) << 15);
		code += (MASK11_0(lower5bit(pArg3, lineNum)) << 20);
        ...
        code = (0x0D << 2) + 0x03;
        code += (reg2num(pArg1, lineNum)<<7);
		code += (MASK11_0(validate_imm(pArg3, 20, lineNum)) << 12);
	...
	// Unconditional Jumps
        ...
        code += (reg2num(pArg1, lineNum) << 7);
        code += (0<<12);
        PExpression *ret = parse_parenthesized_expression(pArg2,lineNum);
        code += (reg2num(ret->reg,lineNum)<<15);
        code += (MASK11_0(validate_imm(pArg3, 12, lineNum)) << 20);
        ...
        code= (0x1b<<2)+0x03;
        code += (reg2num(pArg1, lineNum) << 7);
        code += (handle_label_imm(pArg2,pLabel,8,lineNum)<<12);
        code += (handle_label_imm(pArg2,pLabel,1,lineNum)<<20);
        code += (handle_label_imm(pArg2,pLabel,10,lineNum)<<21);
        code += (handle_label_imm(pArg2,pLabel,1,lineNum)<<31);
	...
	// Load and Store Instructions
        ...
        code += (reg2num(pArg1,lineNum)<<20);
        PExpression *ret = parse_parenthesized_expression(pArg2,lineNum);
        code += (reg2num(ret->reg,lineNum)<<7);
        code += ((ret->imm &0x1F)<< 7);
        code += ((ret->imm & 0xFE0) << 20);
